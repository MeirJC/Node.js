// first errors:
// names, born and meatItem aren't used.
const data = [
  {
    name: "John",
    birthday: "1-1-1995",
...
const food = (arr) => {
  ...
  arr.forEach((el) => {
    el.favoriteFoods.forEach((foodObj) => {
      ...
      //=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
      // we fix it by adding .meats before the .foreach
      // foodObj.forEach((meatItem) => { // <=== This is not working because meats is not an array. we need to address each meat item
      //=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
